...
public class DungeonGenerator : MonoBehaviour {
    ...
    public int dungeonSet = 100;

    public int seed ;
    public bool randomizeSeedOnStart = true;
    public bool randomizeRoomSize;
    public DRandom random;
    public bool randomGenerateDungeonSet = true;

    public bool generationComplete = false;
    public int targetRooms = 500;
    public int roomsCount = 125;
	...
	void Start () {
        ...
        if (randomizeSeedOnStart) {
            seed = Random.Range(0, 1500000000);
        }
        if(randomGenerateDungeonSet)
        {
            dungeonSet = Random.Range(0, 1500000000);
        }
    ...
    private void StartGeneration() {
        ...
        for (int i = 0; i < rooms.Count; i++) {
            for (int j = 0; j < rooms[i].doors.Count; j++) {
                ...
                {
                    Door d = ((GameObject)Instantiate(data.sets[dungeonSet].doors[1].gameObject)).GetComponent<Door>();
                    doors.Add(d);
                    rooms[i].doors[j].door = d;
                    rooms[i].doors[j].sharedDoor.door = d;
                    //
    ...
    private void GenerateNextRoom() {
        ...
        do {
            ...
            GameObject tryDiffRoom;
            GameObject newerRoom;
            int r = random.range(0, possibleRooms.Count - 1);
            int l = random.range(0, possibleRooms.Count - 1);
            int x = random.range(0, possibleRooms.Count - 1);
            tryDiffRoom = possibleRooms[l].gameObject;
            newerRoom = possibleRooms[l].gameObject;
            ////Debug.Log("r: " + r);
            ...
            if(newRoom == lastRoom)
            {
                newRoom = (GameObject)Instantiate(tryDiffRoom);
            }
            if(newRoom == lastRoom)
            {
                newRoom = ((GameObject)Instantiate(newerRoom));
            }
